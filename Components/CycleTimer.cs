...
namespace Fiourp
{
    public class CycleTimer : Component
    {
        ...
        public CycleTimer(float time, int? numberOfCycles, Action<Timer> updateAction, Action<int> onCycle)
        {
            ...
            timer = new Timer(time, (timer) => OnUpdate?.Invoke(timer), () => { onCycle?.Invoke(0); RestartTimer(); });
        ...
        private void RestartTimer()
        {
            ...
            {
                cycle++;
                timer = new Timer(timer.MaxValue, (timer) => OnUpdate?.Invoke(timer), () => { OnCycle?.Invoke(cycle); RestartTimer(); });
            }
            else
